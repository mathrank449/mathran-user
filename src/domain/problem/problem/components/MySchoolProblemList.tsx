...
function MySchoolProblemList() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setSelectedSchool(userInfo.schoolDetail);
    };
    fetchData();
  }, []);
  ...
  return (
    <div className="flex flex-col items-center gap-8">
      ...
      {selectedSchool?.schoolCode ? (
        ...
        <div className="flex justify-center my-6">
          <button
            ...
            className="cursor-pointer px-6 py-3 rounded-2xl bg-gradient-to-r from-emerald-400 to-green-500 text-white font-semibold shadow-md hover:shadow-lg hover:scale-105 transition-transform duration-200"
          >
            내 페이지에서 학교 설정을 해주세요
          </button>
        </div>
      ...
      {selectedSchool?.schoolCode && problemListPagination && (
        <Pagination
          setPage={setPage}
          pageInfo={{
...
export default MySchoolProblemList;
